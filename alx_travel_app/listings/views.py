...
class BookingViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self) -> Any:
        ...
        user_id = self.request.GET.get(user_id=user_id)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.status == "confirmed":
            return Response({"detail": "Cannot delete a confirmed booking."},
...
class PaymentViewSet(viewsets.ReadOnlyModeViewSet):
    """API endpoint fr viewing payments."""
    serializer_class = PaymentSerializer
    permission_classes = [IsAuthenticated]
    lookup_field = "id"

    def get_queryset(self) -> Any:
        """Filter payments to show only user's own payment unless admin."""
        queryset = Payment.objects.all().order_by("-created_at")

        # Non-admin uders can only see their own payments
    ...
    def initiate(self, request):
        ...
        try:
            vbooking = get_object_or_404(Booking, id=booking_id, user=request.user)
    ...
    def status(self, request):
        ...
        tr_ref = request.query_params.get('tr_ref')

        if not tr_ref:
            return Response(
                    {"error": "tr_ref parameter is required"},
                    status=status.HTTP_400_BAD_REQUEST
                    )
        try:
            payment = Payment.objects.select_related('booking', 'booking__user', 'booking__listing').get(booking_reference=tx_ref)
